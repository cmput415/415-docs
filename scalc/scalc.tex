...
\begin {itemize}
  ...
  similar to a \textit{C} \code{while} loop.
	\item
    \code{statement-*} is any type of statement. This can be nested loops, declarations, etc. There
    does not have to be any statements in the loop, but without side effects a loop will be
...
\code{_format_string} deserves a quick explanation. The \code{DB} declares a datatype of bytes,
...
\href{null terminator}{https://en.wikipedia.org/wiki/Null-terminated_string} for the string.

You won't need to know more the \textit{x86} calling conventions than what was demonstrated above.
