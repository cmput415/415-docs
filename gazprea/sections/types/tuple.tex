...
The number of fields in a \code{tuple} must be known at compile. The only exception is when a
\hyperref[sec:typeQualifiers]{variable is declared without a type using \code{var} or \code{const}}.
...
The following operations are defined on \code{tuple} values. In all of the usage examples
\code{tuple-expr} means some \code{tuple} yielding expression, while \code{int_lit} is an
\code{integer} literal as defined in \hyperref[sssec:integer_lit]{Integer Literals} and \code{id}
...
Note that in the above table \code{tuple-expr} may refer to only a variable for access. Accessing a
literal could be replaced immediately with the scalar inside the \code{tuple} literal. However
\code{tuple-expr} may refer to a literal in comparison operations to enable shorthand like
this:
\begin{lstlisting}
  if ((a, b) == (c, d))
  fi;
