...
\begin{enumerate}
  ...
  \item
    ...
    \end{lstlisting}
  \item
    \assertiondest{const-arg}
    Procedure \code{var} arguments will not be passed \code{const} values. A \code{var} argument
    implies the value is mutable in the calling context, which a \code{const} value is not. See
    \nameref{sec:procedure} for more information. For example, the following tests would be
    considered invalid:
    \begin{lstlisting}
  ...
  \item
    ...
    \end{lstlisting}
  \item
    \assertiondest{limited-promotion}
    Implicit type conversions not found in \nameref{sec:typePromotion} will not be performed. This
