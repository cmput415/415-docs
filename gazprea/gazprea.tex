...
\hspace{.7in}
	...
	\textit{Gazprea} is derived from a language originally designed at the IBM Hardware Acceleration Laboratory in Markham, ON.


\section{List of Keywords}
	\textit{Gazprea} has a number of built in keywords that are reserved and should not be overwritten by the programmer.
	\begin{itemize}
		\item in
		\item by
		\item as
...
\section{Identifiers}\label{sec:identifiers}
	Identifiers in \textit{Gazprea} must start with either an underscore, or a letter (either upper or lower cased).
	Subsequent characters in an identifier may be any letter, digit, or an underscore. An identifier may not be any of
	\textit{Gazprea}'s keywords. Here are some valid identifiers in \textit{Gazprea}:
	...
	The following are some examples of invalid identifiers. They either contain invalid characters, or they are
	keywords.
...
\section{Comments}\label{sec:comments}
	\textit{Gazprea} supports \textit{C99} style comments, Single line comments are made using \textbf{//}. Anything on
	the line after two adjacent forward slashes is ignored.

	\begin{lstlisting}
		x = 2 * 3;  // This is ignored
	\end{lstlisting}

	\textbf{/*} and \textbf{*/} can also be used for block comments in \textit{Gazprea>}. These comments can span multiple
	lines. The start of a block comment is marked using \textbf{/*}, and the end of the block comment is the
	\textbf{first} occurrence of the sequence of characters \textbf{*/}. For Example:

	\begin{lstlisting}
		/* This is a block comment. I can span as many lines as we want, and
		   only ends when the closing sequence is encountered.
		 */
		x = 2 * 3;  /* Block comments can also be on a single line */
	\end{lstlisting}

	Mismatched block comment delimiters should raise an error, For instance if you start a block comment without closing
	it, or having a closing */ without starting a comment.
