...
\subfile{sections/typeCasting.tex}

\section{Type Promotion}
\label{sec:typePromotion}
\subfile{sections/typePromotion.tex}
...
\section{Procedures}\label{sec:procedure}
  ...
  \subsection{Syntax}
    ...
    can cause side effects, it makes sense to have procedures without a return value. If a returns clause
    is present, then a return statement must be reached by all possible control flows in the procedure before the
    end of the procedure is encountered. For instance:
...
All developers are advised to read the full
  ...
  \begin{enumerate}[label*=\arabic*]
    ...
    \begin{enumerate}[label*=.\arabic*]
      ...
      \begin{enumerate}[label*=.\arabic*]
        \item \nameref{ssec:typeQualifiers_var}
        \item \nameref{ssec:typeQualifiers_const}
      \end{enumerate}
