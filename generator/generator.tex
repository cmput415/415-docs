...
\begin{center}
  \begin{tabular}{|l|c|l|c|}
    ...
    \textbf{Associativity} \\
    \hline
    exponentiation & \textasciicircum & \code{expr \textasciicircum\ expr} & right\\ \hline
    multiplication & *  & \code{expr * expr}  & left \\
    division       & /  & \code{expr / expr}  & left \\
    remainder      & \% & \code{expr \% expr}  & left \\ \hline
...
\begin{itemize}
  ...
  \item
    ...
    \item
      ...
      yourself (and the marker) to understand why. Consider reading
      \href{https://softwareengineering.stackexchange.com/a/167751}{this answer} for more info.
    \item
      Now that you've separated all of these files, you should make sure they're organised. The
      project is already set up to let you separate your headers from your source. A good idea
      is to have identical directory structures under \code{src} and \code{include} with headers
      and source files mirroring each other (e.g. \code{include/dir/dir2/class.h} and
      \code{src/dir/dir2/class.cpp}). You will be able to include the header via
      \code{\#include "dir/dir2/class.h"}.
    \item
      This is your work and you should own that. Add your name to the README and License as well.
      Your project is not "GeneratorBase". Change the base \code{CMakeLists.txt}'s project name
      to something more appropriate. "Generator", "<ccid>Generator" or something equally
      descriptive should be more appropriate. Just don't change the executable name!
    \item
      You will also need to add your source files in the src/CMakeLists.txt file (or new
      subdirectory \code{CMakeLists.txt}s that you make yourself). You should be explicit about
      your file paths (use \code{CMAKE\_CURRENT\_SOURCE\_DIR} to make them absolute).
      \href{https://cmake.org/Wiki/CMake_Useful_Variables}{These variables} may be helpful.
    \item
      This assignment is not as demanding as later assignments so a lack of documentation and
      information might be understandable. However, in the future, I expect to see appropriate
      comments in source and headers, changes to README where applicable, using the issue tracker,
      etc.
    \end{itemize}
  \item
